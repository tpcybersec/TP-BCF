...
class RSACipher:
	...
	def RSAPrivateKey(self, PrivateKey):
		return KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(base64.b64decode(PrivateKey.replace("-----BEGIN PRIVATE KEY-----", "").replace("-----END PRIVATE KEY-----", "").replace("\r", "").replace("\n", ""))))
	...
	def decrypt(self, CipherText, PrivateKey=None, PublicKey=None, OAEPHashAlg=None, MGFHashAlg=None):
		...
		PlainText = instance.doFinal(CipherText).tostring()

		if self.algorithm == "RSA/ECB/NoPadding":
			PlainText = PlainText.lstrip("\x00")
